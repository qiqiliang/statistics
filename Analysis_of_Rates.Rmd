...
spo_rate_op <- spo_rate %>%
  ...
  # calculated by taking AvgP_Below_1.00x_FPL in full weight, AvgP_Below_1.50x_FPL in 2/3 weight, AvgP_Below_2.00x_FPL
  # in 1/3 weight, and people above AvgP_Below_2.00x_FPL in 0 weight.

# Color by poverty index
spo_plot <-ggplot(spo_rate_op, aes(x = Avg_Prescr_Rate, y = Avg_Suicide_Rate, color=Poverty_Index)) + 
  geom_point()

mid<-mean(spo_rate_op$Poverty_Index)
spo_plot+scale_color_gradient2(midpoint=mid, low="green", mid="white",
                     high="red", space ="Lab" )
```

```{r}
ggplot(spo_rate_op, aes(x = Avg_Prescr_Rate, y = Poverty_Index)) +
  geom_point(alpha = 0.3)+
  geom_smooth(method = lm, se = FALSE) 

m_spi <- lm(Poverty_Index ~ Avg_Prescr_Rate, data = spo_rate_op)
...
m_spi <- lm(Avg_Prescr_Rate ~ Poverty_Index + 
               Avg_Suicide_Rate, data = spo_rate_op)
...
```{r}
# ggplot(spo_rate_op, aes(x = Avg_Prescr_Rate, y = Avg_Suicide_Rate, color=AvgP_Below_1.00x_FPL)) + 
#   geom_point()+
#   scale_color_gradient2(midpoint=mean(spo_rate_op$AvgP_Below_1.00x_FPL), low="green", mid="white",
#                      high="red", space ="Lab" )
# ggplot(spo_rate_op, aes(x = Avg_Prescr_Rate, y = Avg_Suicide_Rate, color=AvgP_Below_1.50x_FPL)) + 
#   geom_point()+
#   scale_color_gradient2(midpoint=mean(spo_rate_op$AvgP_Below_1.50x_FPL), low="green", mid="white",
#                      high="red", space ="Lab" )
# ggplot(spo_rate_op, aes(x = Avg_Prescr_Rate, y = Avg_Suicide_Rate, color=AvgP_Below_2.00x_FPL)) + 
#   geom_point()+
#   scale_color_gradient2(midpoint=mean(spo_rate_op$AvgP_Below_2.00x_FPL), low="green", mid="white",
#                      high="red", space ="Lab" )
```
