...
```{r, label = "boostrap ci suicide and opioids"}
so_boot <- spo_rate %>% 
  specify(Avg_Suicide_Rate ~ Avg_Prescr_Rate) %>%
...
```{r, label = "boostrap ci 1xFPL and opioids"}
sp1_boot <- spo_rate %>% 
  specify(AvgP_Below_1.00x_FPL ~ Avg_Prescr_Rate) %>%
  generate(reps = 1000, type = "bootstrap") %>% 
  calculate(stat = "slope")
...
```{r, label = "boostrap ci 1.5xFPL and opioids"}
sp1_5_boot <- spo_rate %>% 
  specify(AvgP_Below_1.50x_FPL ~ Avg_Prescr_Rate) %>%
  generate(reps = 1000, type = "bootstrap") %>% 
  calculate(stat = "slope")
...
```
```{r, label = "boostrap ci 2xFPL and opioids"}
sp2_boot <- spo_rate %>% 
  specify(AvgP_Below_2.00x_FPL ~ Avg_Prescr_Rate) %>%
  generate(reps = 1000, type = "bootstrap") %>% 
  calculate(stat = "slope")
...
```
```{r, label = "boostrap ci poverty index and opioids"}
si_boot <- spo_rate %>% 
  specify(Poverty_Index ~ Avg_Prescr_Rate) %>%
  generate(reps = 1000, type = "bootstrap") %>% 
  calculate(stat = "slope")
