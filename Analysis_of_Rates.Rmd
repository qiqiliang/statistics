...
# arrange the plots to be next to each other
plot_grid(so_plot, so_rp_plot, so_rp_hplot, ncol = 3)

# get_regression_table(m_so)
# summary(m_so)

b1_so <- lm(Avg_Suicide_Rate ~ Avg_Prescr_Rate, data = spo_rate)$coefficients[2]
...
```{r, fig.width=10, label = "regression analysis below 1xFPL and opioids"}
...
# arrange the plots to be next to each other
plot_grid(sp1_plot, sp1_rp_plot, sp1_rp_hplot, ncol = 3)

# get_regression_table(m_sp1)
# summary(m_sp1)

b1_sp1 <- lm(AvgP_Below_1.00x_FPL ~ Avg_Prescr_Rate, data = spo_rate)$coefficients[2]
```
...
```{r, fig.width=10, label = "regression analysis below 1.5xFPL and opioids"}
...
# arrange the plots to be next to each other
plot_grid(sp1_5_plot, sp1_5_rp_plot, sp1_5_rp_hplot, ncol = 3)

# get_regression_table(m_sp1_5)
# summary(m_sp1_5)

b1_sp1_5 <- lm(AvgP_Below_1.50x_FPL ~ Avg_Prescr_Rate, data = spo_rate)$coefficients[2]
```
...
```{r, fig.width=10, label = "regression analysis below 2xFPL and opioids"}
...
# arrange the plots to be next to each other
plot_grid(sp2_plot, sp2_rp_plot, sp2_rp_hplot, ncol = 3)

# get_regression_table(m_sp2)
# summary(m_sp2)

b1_sp2 <- lm(AvgP_Below_2.00x_FPL ~ Avg_Prescr_Rate, data = spo_rate)$coefficients[2]
```
Opioid prescription rate may be a statistically significant predictor of the percentage of families 
...
# arrange the plots to be next to each other
plot_grid(spi_plot, spi_rp_plot, spi_rp_hplot, ncol = 3)

# get_regression_table(m_spi)
# summary(m_spi)

b1_spi <- lm(Poverty_Index ~ Avg_Prescr_Rate, data = spo_rate)$coefficients[2]
```
...
```{r, label = "null distribution below 1xFPL and opioids"}
sp1_null <- spo_rate %>% 
...
```{r, label = "null distribution below 1.5xFPL and opioids"}
sp1_5_null <- spo_rate %>% 
...
```{r, label = "null distribution below 2xFPL and opioids"}
sp2_null <- spo_rate %>% 
...
```{r, label = "boostrap distribution below 1xFPL and opioids"}
so_boot <- spo_rate %>% 
  specify(Avg_Suicide_Rate ~ Avg_Prescr_Rate) %>%
