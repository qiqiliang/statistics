---
title: "Analysis of Opioid Prescription, Poverty, and Suicide Levels"
...
[poverty rates by county](https://data.hrsa.gov/tools/data-explorer?ds=29,34), and 
[opioid prescription rate per county](https://www.cdc.gov/drugoverdose/maps/rxrate-maps.html). 
The suicide rate averages were calculated per 100,000 people between 2008 and 2014. Poverty rates 
are percentages of families in the county below intervals of the Federal Poverty Level between 2014 
and 2018. The opioid prescription rate averages were calculated per 100 people between 2005 and 2015.  
...
b. State what a Type I error (reject $H_0$ due to unlikely event) and a Type II (reject $H_A$ due to mistaking for unlikely event) error represent in the context of the problem.
...
get_regression_points(m_so) %>% 
  ggplot( aes(x =Avg_Prescr_Rate, y = residual) )+
    geom_point() +
    geom_smooth(method = "lm", se = FALSE)
...
```
```{r}
ggplot(spo_rate, aes(y = Avg_Prescr_Rate, x = Avg_Suicide_Rate)) +
  geom_point()+
  geom_smooth(method = lm, se = FALSE) 

m_so <- lm(Avg_Prescr_Rate ~ Avg_Suicide_Rate, data = spo_rate)
